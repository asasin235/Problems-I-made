...
int main() {
    ...
    for (int T = 1; T <= t; T++) {
        ...
        int isNegative = getRandomFromRange(0, 1);
        if (isNegative) {
            int digitC = 0;
            s += '-';
            for (int i = 1; i < 2997; i++) {
                if (digitC < 8) {
                    s += (char)getRandomFromRange(35, 122);
                } else
                    s += (char)getRandomFromRange(58, 122);
                if (s[s.size() - 1] >= '0' and s[s.size() - 1] <= '9') digitC++;
            }
        } else {
            int digitC = 0;
