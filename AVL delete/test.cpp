...
int main() {
    ...
    ofstream myfile("testcases/large/in/input6.txt");
    ...
    while (t--) {
        ...
        for (int i = 0; i < n; i++) {
            int t = getRandomFromRange(-100, 100);
            if (t == -1) t++;
            arr[i] = t;
            //t++;
        }
	...
	*/
        ...
        myfile << endl;
        myfile << levelOrder[getRandomFromRange(0, levelOrder.size() - 1)] << endl;
        mirrorTree(root);
